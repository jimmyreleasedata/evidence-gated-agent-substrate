...
The released claim-to-artifact map is `validation/paper_claim_to_artifact_map.csv`. Paths with `location=hf_dataset` resolve under the Hugging Face dataset root. Paths with `location=github_repo` resolve under this code repository. Decision-study rows are separate from the 930-row canonical substrate aggregate.
